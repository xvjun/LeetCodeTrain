...
public class Dijkstra<Weight extends Number & Comparable> {
    ...
    public Dijkstra(WeightedGraph g,int s){
        // 算法初始化
        G = g;
        assert s >= 0 && s < G.V() : String.format("s : %d need in [0,%d)",s,G.V());
        this.s = s;
    ...
    public Number shortestPathTo(int w){
        assert w >= 0 && w < G.V() : String.format("w : %d need in [0,%d)",w,G.V());
        assert hasPathTo(w) : String.format("s : %d not to w : %d",s,w);
        return distTo[w];
    }
